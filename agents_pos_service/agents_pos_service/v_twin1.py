...
class RTPositionSubscriber(Node):
    ...
    def __init__(self):
        ...
        super().__init__('rt_position_'+ str(0))
        self.subscription = self.create_subscription(
            VehicleOdometry,
            '/iris'+str(0)+'/fmu/vehicle_odometry/out',
            self.listener_callback,
            10)
        self.subscription  # prevent unused variable warning
...
class CoordinatesPublisher(Node):
    ...
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(Posmsg, 'iris'+str(0)+"/pos", 10)
...
def goToDestination(x, y, z):
    ...
    sendz = -(DISTANCE_FROM_TERRAIN - mh/2 + sendz)

    print(str(MY_ID) + "> Going to destination... (x: %s, y: %s, z: %s)" % (str(sendx), str(sendy), str(sendz)))
    dest_x = sendx
    dest_y = sendy
    dest_z = sendz
    coordinates_publisher.send_coordinates() #TODO uncomment this line
